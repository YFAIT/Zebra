...
namespace SurfaceTrails2.FlockingInBox
{
    public class GhcFlockingSimulation : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            MeshWindForming meshWindForming = new MeshWindForming();
            var boxContainment = new BoxContainment();
            var alignment = new Alignment();
            var cohesion = new Cohesion();
            var separation = new Separation();
            var repeller = new Repellers();
            var attractor = new Attractors();
            var attractorCurve = new AttractorCurve();
            var windVector = new Wind();
            ...
            foreach (FlockAgent agent in agents)
            {
                ...
                //agent.Behaviours.Add(alignment);
                //agent.Behaviours.Add(cohesion);
                //agent.Behaviours.Add(separation);
                //agent.Behaviours.Add(repeller);
                //agent.Behaviours.Add(attractor);
                //agent.Behaviours.Add(attractorCurve);
                //agent.Behaviours.Add(windVector);

                agent.IContainment2 = meshWindForming;
                meshWindForming.Mesh = mesh;
            ...
            else
            {
                ...
                flockSystem.CurvePoints = startPoints;
                flockSystem.ClosestPoints = closestPoints;
