...
namespace SurfaceTrails2.Composite
{
    public class Composite : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DataTree<object> partitionedTree = new DataTree<object>();
            for (int p = 0; p < sortedPoints.BranchCount; p++)
            {
                var path = sortedPoints.Path(p);
                var list = sortedPoints.Branch(p);
               ...
               for(int q =0;q<list.Count;q++)
                {
                    partitionedTree.Add(list[q], new GH_Path(p, index));
                    count++;
                    if (count>=8)
                    {
                        count = 0;
