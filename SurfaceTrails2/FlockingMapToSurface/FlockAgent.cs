...
namespace SurfaceTrails2.FlockingMapToSurface
{
    public class FlockAgent
    {
        ...
        public void ComputeDesiredVelocity(List<FlockAgent> neighbours)
        {
            ...
            if (Position.X < FlockSystem.XMin)
            desiredVelocity += new Vector3d((FlockSystem.XMax - Position.X)* bounceMultiplier, 0.0, 0.0);

            else if (Position.X > FlockSystem.XMax)
                desiredVelocity += new Vector3d(-Position.X* bounceMultiplier, 0.0, 0.0);


            if (Position.Y < FlockSystem.YMin)
                desiredVelocity += new Vector3d(0.0, (FlockSystem.YMax - Position.Y)* bounceMultiplier, 0.0);

            else if (Position.Y > FlockSystem.YMax)
                desiredVelocity += new Vector3d(0.0, (-Position.Y)* bounceMultiplier, 0.0);


            //if (Position.Z < 0.0)
            //    desiredVelocity += new Vector3d(0.0, 0.0, -Position.Z);
            //else if (Position.Z > FlockSystem.XMax)
            //    desiredVelocity += new Vector3d(0.0, 0.0, FlockSystem.XMax - Position.Z);
